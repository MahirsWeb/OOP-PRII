...
public:
	...
	T1& operator[](T2 vrijednost)
	{
		for (int i = 0; i < *_trenutno; i++)
		{
			if (*_elementi2[i] == vrijednost)
				return *_elementi1[i];
		}
	}
	~Kolekcija() {
...
bool VeciManji(Datum& postoji, Datum& novi) {
	if (postoji.toDays() > novi.toDays())
		return true;
	return false;
}
class Tehnika {
...
public:
	...
	*/
	bool AddOcjena(int ocjena, Datum datum)
	{
		...
		{
			if (razlikaDana(datum, *_ocjene.getElement2(trenutno - 1)) < 3)
				return false;
			if (VeciManji(*_ocjene.getElement2(trenutno - 1), datum))
				return false;
		}
		_ocjene.AddElement(ocjena, new Datum(datum));
		return true;
	}
	float prosjekTehnike() {
		float prosjek = 0.0f;
	...
	friend ostream& operator<<(ostream& COUT, Tehnika& obj) {
		...
		COUT << "Prosjek-> " << obj.prosjekTehnike() << endl;
		COUT << crt << endl;
		return COUT;
	}
...
public:
	...
	friend ostream& operator<< (ostream& COUT, const Polaganje& obj) {
		COUT << endl;
		COUT << " : : : POJAS & TEHNIKE : : : " << endl;
		COUT<< "Pojas-> " << ispisPojas[obj._pojas] << endl;
		COUT << endl;
		COUT << " : : : TEHNIKE & OCJENE : : : " << endl;
		for (size_t i = 0; i < obj._polozeneTehnike.size(); i++)
...
class KaratePolaznik : public Korisnik{
	...
	*/
	void saljiMail(Pojas pojas, Tehnika tehnika)
	{
		muteks.lock();
		cout << endl;
		cout << "FROM: info@karate.ba" << endl;
		cout << "TO: " << GetEmail() << endl;
		cout << "Postovani " << GetImePrezime() << ", evidentirana vam je tehnika " << tehnika.GetNaziv() << " za " << ispisPojas[pojas] << " pojas.";
		cout << "Dosadasnji uspjeh na pojasu " << ispisPojas[pojas] << " iznosi ";
		for (int i = 0; i < _polozeniPojasevi.size(); i++)
		{
			if (pojas == _polozeniPojasevi[i]->GetPojas())
				cout << _polozeniPojasevi[i]->prosjekPojasa();
		}
		cout << ", a ukupni uspjeh na svim pojasevima iznosi " << prosjekUkupno() << ".\n" << "Fakultet informacijskih tehnologija\n" << "Pozdrav.\n" << "KARATE Team." << endl;
		cout << endl;
		muteks.unlock();
	}
public:
	...
	float prosjekUkupno() {
		...
		for (int i = 0; i < _polozeniPojasevi.size(); i++)
		{
			prosjek += _polozeniPojasevi[i]->prosjekPojasa();
		}
	...
	bool AddTehniku(Pojas pojas, Tehnika tehnika)
	{
		for (int i = 0; i < _polozeniPojasevi.size(); i++)
		{
			...
			if (pojas == _polozeniPojasevi[i]->GetPojas())
			{
				...
				thread mail(&KaratePolaznik::saljiMail, this, pojas, tehnika);
				mail.join();
				return true;
			}
		}
		...
		_polozeniPojasevi.push_back(new Polaganje(pol));
		thread mail(&KaratePolaznik::saljiMail, this, pojas, tehnika);
		mail.join();
		return true;
	}
